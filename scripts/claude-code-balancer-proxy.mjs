...
const PRESSURE_EARLY_BALANCE_RATIO = 0.7;
const PRESSURE_BALANCE_RATIO = 0.8;
const PRESSURE_CONSTRAINED_RATIO = 0.9;
...
function discoverKeySpecs() {
  ...
  for (const [name, value] of Object.entries(process.env)) {
    ...
    if (suffix.endsWith("_BALANCE_ID") || suffix.endsWith("_LIMIT") || suffix.endsWith("_FALLBACK"))
      continue;
    if (!value) continue;
    specs.push({
      suffix,
...
function ensureStateShape(state) {
  return {
    ...
    shadowQuota:
      state?.shadowQuota && typeof state.shadowQuota === "object" ? state.shadowQuota : {},
  };
}
...
async function queryLiveBalances(
  upstreamBaseUrl,
  quotaAuth,
  keys,
  balanceCache,
...
function maxPressure(snapshots) {
  return snapshots.length ? snapshots.reduce((max, item) => Math.max(max, item.pressure), 0) : 0;
}
...
function shortWindowForwardCount(forwards, keyId, currentTime = nowMs(), windowMs = 60_000) {
  return (forwards || []).filter(
    (forward) => forward.keyId === keyId && currentTime - Number(forward.at || 0) <= windowMs,
  ).length;
}
...
function computeLiveBalanceTtlMs(snapshots) {
  const totalRemaining = totalRemainingCalls(snapshots);
  if (totalRemaining <= HIGH_PRESSURE_TOTAL_REMAINING_THRESHOLD)
    return LIVE_BALANCE_TTL_HIGH_PRESSURE_MS;
  const hottest = maxPressure(snapshots);
...
function snapshotEligibility(snapshot) {
  if (
    snapshot.hardLimit ||
    snapshot.remainingCalls <= 0 ||
    snapshot.pressure >= PRESSURE_HARD_RATIO
  ) {
    return "hard-blocked";
  }
...
function shouldUseFallbacks(snapshots) {
  ...
  return primarySnapshots.every(
    (item) =>
      item.pressure >= PRESSURE_CONSTRAINED_RATIO || item.remainingCalls <= 0 || item.hardLimit,
  );
}
...
function pickRecommendation(snapshots, forcedKeyLabel = "", forwards = []) {
  ...
  const scoredSnapshots = eligibleSnapshots
    ...
    .map((item) => {
      ...
      let score = ratio;
      if (item.pressure >= PRESSURE_EARLY_BALANCE_RATIO)
        score -= (item.pressure - PRESSURE_EARLY_BALANCE_RATIO) * 0.8;
      if (item.pressure >= PRESSURE_BALANCE_RATIO)
        score -= (item.pressure - PRESSURE_BALANCE_RATIO) * 1.4;
      if (item.pressure >= PRESSURE_CONSTRAINED_RATIO)
        score -= (item.pressure - PRESSURE_CONSTRAINED_RATIO) * 2.4;
...
function recommendParallelism(snapshots) {
  ...
  const healthyPrimaryKeys = snapshots.filter(
    (item) =>
      !item.fallbackOnly &&
      !item.hardLimit &&
      item.remainingCalls > 0 &&
      item.pressure < PRESSURE_CONSTRAINED_RATIO,
  ).length;
...
function buildStatusPayload({
  ...
  const allLive =
    queriedBalance &&
    snapshots.length > 0 &&
    snapshots.every((item) => item.source === "live-balance");
  ...
  const lastForward =
    [...(forwards || [])].sort((a, b) => Number(b.at || 0) - Number(a.at || 0))[0] || null;
  ...
  const enrichedSnapshots = snapshots.map((snapshot) => ({
    ...
    snapshotAgeMs:
      snapshot.source === "live-balance" && Number.isFinite(snapshot.queriedAt)
        ? Math.max(0, now - snapshot.queriedAt)
        : null,
  }));
  if (!allLive) {
    warnings.unshift(
      "Live balance is unavailable or incomplete; current load numbers are not trustworthy.",
    );
  }
  if (highPressureMode) {
    warnings.unshift(
  ...
  return {
    ...
    balancer: {
      ...
      healthyPrimaryKeys: snapshots.filter(
        (item) => !item.fallbackOnly && snapshotEligibility(item) === "preferred",
      ).length,
    },
    warnings,
  };
...
function normalizeUpstreamBaseUrl(value) {
  ...
  if (isLoopbackHost && String(url.port || "") === String(DEFAULT_PORT)) {
    throw new Error(
      `Refusing to use local balancer URL as upstream (${url.origin}). Set ANTHROPIC_UPSTREAM_BASE_URL to the real upstream endpoint.`,
    );
  }
...
function recordForward(state, recommendation, req) {
  ...
  if (
    state.shadowQuota?.[recommendation.keyId] &&
    Number.isFinite(state.shadowQuota[recommendation.keyId].remainingCalls)
  ) {
    state.shadowQuota[recommendation.keyId].remainingCalls = Math.max(
      0,
      state.shadowQuota[recommendation.keyId].remainingCalls - 1,
    );
...
async function main() {
  ...
  async function getSnapshots() {
    ...
    const liveBalances = await queryLiveBalances(
      upstreamBaseUrl,
      quotaAuth,
      keys,
      balanceCache,
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (!status.trustedLoad) {
        writeJson(res, 503, {
          error:
            "Live balance unavailable or incomplete; refusing to route on untrusted load data.",
          warnings: status.warnings,
      ...
      if (!selectedKey) {
        writeJson(res, 500, {
          error: `Selected key ${status.recommendation.label} missing secret`,
        });
        return;
      }
