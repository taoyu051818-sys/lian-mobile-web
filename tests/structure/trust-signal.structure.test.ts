...
const feedCardSource = fs.readFileSync(path.join(repoRoot, "src/features/feed/FeedItemCard.vue"), "utf8");
...
describe("trust signal structure", () => {
  ...
  it("forwards feed source and actor identity without changing absent items", () => {
    expect(feedCardSource).toMatch(/function resolveTrustSignal\(source: FeedItem\["source"\], identityTag\?: string\)/);
    ...
    expect(feedCardSource).toMatch(/if \(identityTag\) return `\$\{TRUST_SIGNAL_IDENTITY_PREFIX\}\$\{identityTag\}`;/);
    expect(feedCardSource).toMatch(/if \(source\) return TRUST_SIGNAL_UNKNOWN;/);
    expect(feedCardSource).toMatch(/return null;/);
    expect(feedCardSource).toMatch(/trustSignal: resolveTrustSignal\(item\.source, actor\.identityTag\)/);
    expect(feedShellSource).toMatch(/trustSignal: string \| null;/);
    expect(feedShellSource).toMatch(/:trust-signal="trustSignal"/);
    expect(feedFooterSource).toMatch(/import \{ TrustBadge, VisibilityBadge \} from "\.\.\/\.\.\/ui";/);
  ...
  it("renders detail and replies conditional badges with safe fallback", () => {
    ...
    expect(detailPanelSource).toMatch(/if \(post\.value\?\.source\?\.visible === false\) return null;/);
    expect(detailPanelSource).toMatch(/post\.value\?\.source\?\.label \|\|\s+post\.value\?\.actor\?\.identityTag \|\|\s+\(post\.value\?\.source \? TRUST_SIGNAL_UNKNOWN : null\)/);
    ...
    expect(repliesSource).toMatch(/return reply\.source\?\.label \|\| reply\.actor\?\.identityTag \|\| \(reply\.source \? TRUST_SIGNAL_UNKNOWN : null\);/);
  ...
  it("marks channel thread authoritative actors and identity values", () => {
    expect(channelThreadSource).toMatch(/import \{ EmptyState, InlineError, LianButton, TrustBadge \} from "\.\.\/\.\.\/ui";/);
    expect(channelThreadSource).toMatch(/function messageTrustSignal\(item: ChannelMessage\)/);
    expect(channelThreadSource).toMatch(/if \(actor\.authoritative\) return TRUST_SIGNAL_VERIFIED;/);
    expect(channelThreadSource).toMatch(/return item\.source\?\.label \|\| actor\.identityTag \|\| null;/);
