...
| `features/profile/ProfileAdminLink.vue`          | Mirror the env-gate that lived inline in ProfileView before the IdentityGroup                                            |    50 |
...
| `features/profile/ProfileIdentityGroup.vue`      | Keep the inner ref in step with the prop so the composables (which expect a                                              |   149 |
...
| `features/profile/ProfileView.vue`               | Initial paint emits a no-op chrome spec because the IdentityGroup is gated                                               |   207 |
...
| `types/profile.ts`         | PRD V0.1 §17 — verification records keyed by tag (campus_verified, merchant_verified, ...). Backend `/api/auth/me` re...  |    91 |
