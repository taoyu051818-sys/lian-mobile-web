...
| `api/channel.ts`       | export function extractChannelMessagePlainText(html?: string): string                                                    |   154 |
...
| `api/notifications.ts` | Server-side `type` slugs that B2 (#438 / lian-platform-server#445) writes for the three event-lifecycle fan-outs. The... |   650 |
...
| File     | Summary                                                                                                                 | Lines |
| -------- | ----------------------------------------------------------------------------------------------------------------------- | ----: |
| `app.ts` | Build a fresh Vue app + i18n instance pair. Phase 1.1a of the SSR/PWA RFC (`docs/architecture/SSR_PWA_RFC_2026_05_23... |    22 |
...
| `config/brand/publish.ts`      | export const PUBLISH_CLEAR = "清空"; export const PUBLISH_SUBMIT = "发布"; export const PUBLISH_LOCATION_LABEL = "地点"; e...                         |   120 |
...
## src/entry-client.ts/ (1 files)

| File              | Summary           | Lines |
| ----------------- | ----------------- | ----: |
| `entry-client.ts` | TypeScript module |    19 |

## src/entry-server.ts/ (1 files)

| File              | Summary                                                                                                                 | Lines |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------- | ----: |
| `entry-server.ts` | SSR contract entry — phase 1.2 of `docs/architecture/SSR_PWA_RFC_2026_05_23.md`. `render(url)` is the boundary betwe... |   304 |

## src/features/ (171 files)
...
| `features/detail/PostReplyDock.vue`                | Vue SFC                                                                                                                       |   185 |
...
| `features/errand/ErrandOrderView.vue`              | Destructure refs so the template can read them via auto-unwrap instead of                                                     |   544 |
...
| `features/feed/FeedItemCard.vue`                   | Vue SFC                                                                                                                       |   145 |
...
| `features/feed/FeedItemCardShell.vue`              | Pure-presentation skeleton for the feed/publish card.                                                                         |   232 |
...
| `features/profile/ProfileServerChanBlock.vue`      | Best-effort localized rendering. If the input is a malformed timestamp,                                                       |   621 |
...
| `features/publish/PublishComposer.vue`             | Vue SFC                                                                                                                       |   307 |
...
| `features/publish/PublishTitleCandidateBar.vue`    | Vue SFC                                                                                                                       |   113 |
...
| `features/publish/usePublishDraft.ts`              | Body candidate slot (PRD V0.2 step B). The LLM-polished body lives in `bodyCandidate` until the user explicitly appl...       |   553 |
...
| File                  | Summary                                                                                                                 | Lines |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----: |
| `server/ssr/index.ts` | Node http service for SSR — phase 1.2 of `docs/architecture/SSR_PWA_RFC_2026_05_23.md`. Wraps the route-aware `rende... |    95 |
| `server/ssr/start.ts` | CLI bootstrap for the SSR Node http service. Kept as a separate file from `./index.ts` so that importing the server ... |    31 |
...
| `styles/lian-tokens.css`          | Stylesheet |   110 |
...
| `ui/feedback/ToastHost.vue`  | Vue SFC                                                                                                                 |    58 |
