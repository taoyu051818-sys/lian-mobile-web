...
| `api/posts.ts`        | export interface PostLikeResponse                                                                                        |   146 |
| `api/profile.ts`      | TypeScript module                                                                                                        |    69 |
| `api/publish.ts`      | Creates a display-only fallback draft from free-text place name. When `placeName` is non-empty the source is "manual"... |   243 |
...
| `config/brand/index.ts`        | TypeScript module                                                                                                                                     |    19 |
...
| `features/detail/PostDetailContent.vue`         | Vue SFC                                                                                                                  |   172 |
...
| `features/detail/PostDetailPanel.vue`           | V0.1 surface — emit retry so the panel reloads to the linked-event tid                                                   |   361 |
...
| `features/publish/PublishView.vue`              | PRD §7.4.2 step 4: after the first successful upload, surface the                                                        |   476 |
...
| `features/publish/usePublishDraft.ts`           | Composes the three slices of publish-form state — form fields & uploads (this file), identity (`usePublishIdentity`),... |   273 |
...
| `features/publish/usePublishSubmit.ts`          | export function usePublishSubmit(options:                                                                                |   177 |
...
| File                          | Summary                                                                                                                  | Lines |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----: |
| `platform/api-normalizers.ts` | Coerce to a non-negative integer, defaulting to 0. Convenience wrapper for the count/capacity fields backends sometim... |   261 |
...
| `types/post-extensions.ts` | Event / Help / Merchant / Trade / Errand domain types (PRD V0.1 §6). V0.1 ships shapes only — no UI yet. The point i...   |   166 |
| `types/post.ts`            | event?: EventPostExtension; eventJoined?: boolean; help?: HelpPostExtension; helpVoted?: boolean; helpManageable?: bo...  |   131 |
| `types/profile.ts`         | PRD V0.1 §17 — verification records keyed by tag (campus_verified, merchant_verified, ...). Backend `/api/auth/me` re...  |    56 |
| `types/publish.ts`         | export type PublishMapVersion = "legacy" \| "manual" \| "gaode_v2"; Backend (#383) accepts these contentType values fo... |    95 |
