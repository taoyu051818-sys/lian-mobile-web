...
Recent GitHub update summary: `docs/agent/references/GITHUB_RECENT_UPDATES_2026-05-05.md`

Latest long-thread handoff: `docs/agent/references/RECENT_WORK_HANDOFF_2026-05-05.md`
...
7. **Done / split transition** Backend repo now lives in `lian-platform-server`; full-stack repo is stopped. Remaining repo-split work is documentation alignment and any second-pass frontend cleanup such as `data/*` ownership.
...
```text
Stabilize acceptance, keep Audience write-side enforcement visible, and align docs after the completed backend split. Do not expand product scope or start large refactors now.
```
...
- backend repo bootstrap is no longer future planning: backend is in `lian-platform-server`; remaining work is validation/docs/cleanup alignment.
...
| Task | Status | Owner | Goal | Acceptance |
|---|---|---|---|---|
| `backend-repo-bootstrap-readiness-plan` | **Done / Superseded** | Architecture / DevOps | Backend repo has been created as `lian-platform-server`. | Keep only as historical planning context. |
| `backend-repo-bootstrap-copy-and-validate` | **Done / Superseded** | DevOps + backend owner | Backend runtime has moved to `lian-platform-server`; full-stack repo is stopped. | Future validation belongs in `lian-platform-server`. |
...
4. ~~Bootstrap backend repo without changing runtime behavior.~~ Done: backend lives in `lian-platform-server`.
5. ~~Add frontend API base URL configuration in current repo.~~ Done (Phase 0.5)
6. Stage reverse proxy deployment for split frontend/backend.
7. ~~Remove backend ownership from current repo only after backend repo is validated.~~ Done at code layer; `data/*` cleanup remains follow-up scope.

Status: **Done / split executed** - backend now lives in `lian-platform-server`; frontend lives in `lian-mobile-web`; historical full-stack is stopped. API contract remains the cross-repo boundary. Remaining cleanup should be handled as follow-up docs/data ownership PRs, not as backend bootstrap planning.
