...
function nullablePoint(value) {
  ...
  return {
    x: nullableNumber(input.x),
    y: nullableNumber(input.y)
  };
}
...
function normalizeLocationDraft(value = {}, { aiLocationArea = "", locationHint = "" } = {}) {
  ...
  const locationArea = truncateText(input.locationArea || input.displayName || aiLocationArea || locationHint || "", 40);
  const displayName = truncateText(input.displayName || locationArea, 40);
