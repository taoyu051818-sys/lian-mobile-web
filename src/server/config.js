...
function defaultImageProxyPublicBaseUrl() {
  const port = Number(process.env.IMAGE_PROXY_PORT || 4101);
  const explicit = String(process.env.IMAGE_PROXY_PUBLIC_BASE_URL || "").trim();
...
export const config = {
  ...
  imageProxyPort: Number(process.env.IMAGE_PROXY_PORT || 4101),
...
export async function saveSetupConfig(payload = {}, onSaved = null) {
  const next = {
    ...
    IMAGE_PROXY_PORT: String(Number(payload.imageProxyPort || config.imageProxyPort || 4101)),
    IMAGE_PROXY_PUBLIC_BASE_URL: String(payload.imageProxyPublicBaseUrl || config.imageProxyPublicBaseUrl || "").replace(/\/$/, ""),
  ...
  config.imageProxyPort = Number(next.IMAGE_PROXY_PORT);
