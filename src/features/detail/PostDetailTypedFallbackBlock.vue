...
const FALLBACK_CONFIG: Record<DetailActionFallbackPostType, FallbackConfig> = {
  event: {
    ...
    actionLabel: "暂时无法报名",
    reason:
      "这条内容看起来是活动帖，但详情没有返回活动时间、人数或报名信息，现在还不能判断是否可以参加。",
  },
  help: {
    blockLabel: "求助操作",
    title: "求助信息暂时不可用",
    actionLabel: "暂时无法投票",
    reason:
      "这条内容看起来是求助帖，但详情没有返回求助状态或投票信息，现在还不能判断是否可以支持。",
  },
  merchant: {
    blockLabel: "商家跑腿",
    title: "商家跑腿入口暂时不可用",
    actionLabel: "暂时无法帮我取",
    reason:
      "这条内容看起来是商家帖，但详情没有返回商家信息或跑腿资格，现在还不能判断是否可以下单。",
  },
  trade: {
    blockLabel: "交易操作",
    title: "交易信息暂时不可用",
    ...
    reason:
      "这条内容看起来是交易帖，但详情没有返回价格、状态或交易说明，现在还不能判断是否还能继续联系。",
  },
};
