  ...
  <section class="profile-editor__block" aria-labelledby="profile-alias-title">
    ...
    <div class="profile-editor__alias-list">
      <label class="profile-editor__alias" :class="{ 'is-active': !activeAliasId }">
        <input type="radio" name="profileAlias" value="" :checked="!activeAliasId" :disabled="busy" @change="handleSwitch('')" />
      ...
      >
        <input type="radio" name="profileAlias" :value="alias.id" :checked="alias.id === activeAliasId" :disabled="busy" @change="handleSwitch(alias.id)" />
