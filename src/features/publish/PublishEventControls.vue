 ...
 *
 * Renders the event-only fields: startAt / endAt / capacity / joinPolicy.
 * The "is this an event?" decision is the parent publishKind switch
 * (PR-3 #813 follow-up): when publishKind === "event" PublishView mounts
 * this panel and keeps eventDraft.postType in lock-step, so the existing
 * usePublishSubmit createEvent branch keeps firing without a separate
 * inner post-type chooser.
 *
 * participantScope reuses the existing post-level visibility picker —
 * there's only one audience per post.
 *
