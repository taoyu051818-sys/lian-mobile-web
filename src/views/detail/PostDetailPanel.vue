...
const authorInitial = computed(() => actorAvatarText(props.post?.actor));
const hasAuthorChip = computed(() => Boolean(authorLabel.value || authorAvatarUrl.value || authorInitial.value));
...
const replyIdentityLabel = computed(() => "");
...
function actorAvatarText(actor?: DisplayActor | null) {
  return actor?.avatarText || "";
}
...
<template>
  <aside class="post-detail-panel" aria-labelledby="post-detail-title">
    ...
    >
      ...
      <div v-if="hasAuthorChip" class="post-detail-panel__author-chip">
      ...
      <template v-else-if="post">
        ...
        <section class="post-detail-panel__info-strip" aria-label="帖子属性">
          <div class="post-detail-panel__info-left">
            ...
            >
              {{ placeStatusText ? `${placeLabel} · ${placeStatusText}` : placeLabel }}
            </button>
            ...
            <div v-if="placeSheet?.recentPosts?.length" class="post-detail-panel__place-posts">
              <article v-for="recent in placeSheet.recentPosts.slice(0, 3)" :key="String(recent.tid)">
                ...
                <small v-if="placeRecentPostActorLabel(recent.actor) || formatRelativeTime(recent.timestampISO || '')">
                  <template v-if="placeRecentPostActorLabel(recent.actor)">{{ placeRecentPostActorLabel(recent.actor) }}</template>
                  <template v-if="placeRecentPostActorLabel(recent.actor) && formatRelativeTime(recent.timestampISO || '')"> · </template>
                  <template v-if="formatRelativeTime(recent.timestampISO || '')">{{ formatRelativeTime(recent.timestampISO || '') }}</template>
                </small>
              </article>
            </div>
          </template>
        ...
        <section class="post-detail-panel__replies" aria-labelledby="post-detail-replies-title">
          ...
          <article v-for="reply in replies" :key="String(reply.id)" class="post-detail-panel__reply">
            ...
            <div v-if="stripDecorativeContentFromHtml(reply.content || '')" class="post-detail-panel__reply-content" v-html="stripDecorativeContentFromHtml(reply.content || '')"></div>
          </article>
...
.post-detail-panel__author-chip--empty {
  width: 1px;
  min-height: 32px;
}
